...
const changeComputerHand = ()=>{
    if(computerChoice=== 'r'){computerChoice='s'}
    else if(computerChoice==='s'){computerChoice = 'p'}
    else if(computerChoice==='p'){computerChoice = 'r'}
...
const clickButton = (event)=>{
    if(clickable && playCount<5){
        ...
        clickable = false;
        const myChoice = event.target.textContent === '바위'?'r':event.target.textContent ==='가위'?'s':'p';
